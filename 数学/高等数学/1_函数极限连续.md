   ...
   $$
     \begin{aligned}
      \Rightarrow充分性   \xrightarrow[]{奇函数}&f(x)=-f(-x)\\
        ...
        & f(-x)=\underbrace{\int_0^{-x}f'(t)dt}_{^{t=-u}\int_0^{-x}f'(-u)(-dt)=-\int_0^{x}f'(u)du}+C\\
...
- 数列极限 $\{u_{n}\}收敛于A⇔\lim\limits_{n →∞}u_{n}=A$
  - $∀ϵ>0,∃ N>0,n>N⇒|u_{n}-A|<ϵ$
  ...
  - 极限存在三性质
    ...
    - 保号性
      $A\neq 0$, $\exists\mathring{*}$, $f(x)$与A同号.  
       - 保号性推论
          - $设\exists\mathring{*}\Rightarrow f(x)\geqslant |>(\leqslant |<)0→A\geqslant 0(\leqslant 0)$
...
\begin{aligned}
  x→0 (\neq 0)::~~~~& \frac{\sin x}{x}=1\\
  & (1+x)^{\frac{1}{x}}=e\\
 x→0^{+}::~~~~ &x^{\delta }(\ln x)^{k}=0 \\
 x→\infty::~~~~ & x^{k}e^{-\delta x}=0 ~~~~(k,\delta >0)\\
  n→\infty:: ~~~~& \sqrt[n]{n}=1\\
  & \sqrt[n]{a}=1~~~~(a>0)\\
...
- $\cfrac0{\infty}=0~~~~~~~~(\cfrac0{\infty}=0\cdot\cfrac{1}{\infty}=0\cdot 0)$
- $\cfrac{\infty}0=\infty$
- 洛必达法则
  - $A都0|\infty$
...
- $1^{\infty },0^0,\infty ^0$
    - $1^{\infty }$   
    - $凑成\lim\limits_{x→0} ( 1+x )^{\frac{1}{x}}=e$  

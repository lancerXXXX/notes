...
\\&\int_{a}^{b}f(x)dx=\lim_{\beta \rightarrow b^{-}} \int_{a}^{\beta }f(x)dx(b\text{为}\textcolor{lightgreen}{\text{奇点}})
\\&\text{若}a, b\text{都为奇点}, \textcolor{lightgreen}{\text{应分成}}
...
$$
\begin{aligned}
&\textcolor{lightgreen}{\text{连续}, \text{奇函数},\int_{0}^{+\infty }f(x) dx\text{收敛}}, \rightarrow \int_{-\infty }^{+\infty }f(x) dx=0
...
$$
\begin{aligned}
    & [-a,a]\text{除}\pm c(\text{奇点})\text{连续} \\
    & f(x)\text{奇函数},\int_{0}^{a}\text{收敛}\\ \hdashline
    & \int_{-a}^{a}=0 \\ \hline
    & \text{偶函数} \\ \hdashline
    & 2\text{倍}
\end{aligned}

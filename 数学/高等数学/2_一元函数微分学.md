...
- $\mathring{*}有定义~~~~f'(x_0)\text{或}\frac{df(x)}{dx}\mid_{x=x_0}=\lim_{\Delta x\rightarrow 0} \frac{f(x_0+\Delta x)-f(x_0)}{\Delta x}\text{或}\lim_{x\rightarrow x_0} \frac{f(x)-f(x_0)  }{x-x_0}$
    $$
    \begin{aligned}
    &[\text{注}]\\
...
- 是否可导
  - 利用定义
    ...
    \end{aligned}
    $$
  - $\Leftrightarrow f'_{-}(x_0)=f'_+(x_0)=f'(x_0)$,但是并不代表( $\lim_{x\rightarrow x_0}f'(x)=f'(x)$即$f'(x^{-})=f'(x^{+})=f'(x)$ )
- 可导与连续的关系
  - $可导\Rightarrow 连续$
  - $连续\nLeftarrow 可导$
- 可导与可微的关系
...
- 微积分常见定则
    ...
    &-\cfrac{1}{1+x^2} \\
    \ln (x+\sqrt[]{x^2-a^2})
    &\cfrac{1}{\sqrt[]{x^2-a^2}}\\
    \ln (\tan(u)+\sec(u))
    &\sec(u)\\
    y
    &u(x)^{v(x)}\text{求导}(\text{两边取}\ln , \text{求导}) 
    \end{array}
    $$
- 变限积分
$$
...
<center> <img src="2_%E4%B8%80%E5%85%83%E5%87%BD%E6%95%B0%E5%BE%AE%E5%88%86%E5%AD%A6/2020-04-06-19-57-35.jpg" width=255/> </center>
...
1. 铅直渐近线 $\lim_{x\rightarrow x_0}=\infty$
2. 水平渐近线 $\lim_{x\rightarrow +\infty=a,\ \ \lim_{x\rightarrow -\infty}=b}$
3. 斜渐近线
$$
\begin{aligned}

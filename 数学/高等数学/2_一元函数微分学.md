...
- 拐点
  - ⇒$f''(x_0)存在∧f''(x_{0})=0$
  - 拐点$⇐x_{0}连续∧\mathring{⋃}(x_0)二阶可导∧f''(x)$左右邻域反号
- 渐近线
...
- 柯西中值定理  
  - $[a, b]连续, (a, b)可导, 至少存在一点\xi \\\frac{f(b)-f(a) }{g(b)-g(a)}=\frac{f'(\xi )}{g'(\xi )}$

- 泰勒定理
  - ① $[a,b]$有$n$阶连续的导数,$(a,b)$内有直到$n+1$阶导数,$x_0,x∈[a,b]$,则至少存在一点$ε$介于$x_0,x$之间,使  
  ...
  - ② 减弱条件: $x=x_0$处有$n$阶导数($\rightarrow$某邻域内有$n-1$阶导数 & $f^{(n-1)}(x)$这一点连续),  

...
> 2. 水平渐近线 $\lim_{x\rightarrow +\infty=0,\ \ \lim_{x\rightarrow -\infty}=0}$
> 3. 斜渐近线
...
1. 求导
2. 证明区间内存在极值点(存在比两端函数值大的)  
   1.  积分中值定理
   2. $f'(a)$和$f'(b)$反号$\rightarrow \underbrace{\text{导数定义}}_{\text{设}f'(a)<0\rightarrow\lim_{x\rightarrow a^{+}}\frac{f(x)-f(a)}{x-a}<0,\lim_{x\rightarrow b^{-}}\frac{f(x)-f(b)}{x-b}>0} \rightarrow f(x) -f(a)<0, f(x) -f(b)<0\rightarrow f(a)$和$f(b)$都不是最小值, 所以存在一个极小值点
3. "积分法"
